        ...
        public static bool Prefix(MTFRespawn __instance)
		{
			...
			try
			{
				...
				foreach (var player in EventPlugin.DeadPlayers.ToArray())
                {
                    ...
                    if (player.GetOverwatch() || player.GetRole() != RoleType.Spectator && player.GetRole() != RoleType.Tutorial)
                    {
                ...
                var num = 0;
				foreach (var player in playersToRespawn.TakeWhile(player => num < maxRespawn).Where(player => player != null))
                {
                    ++num;
                    if (isChaos)
                    {
